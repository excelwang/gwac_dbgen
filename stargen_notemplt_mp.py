...
def addExtinctiontoMag(dd, peakval=0.5) :
    """
...
def genOneSimuCatalog(catano, ccd, destdir, templatefile, rac, decc, sqd, wcs_transtool, pixscal=11.7, sizes=[4096,4096], zoneheig=10.):
    """
    ...
    dd=addGaussNoisetoxy(dd , wcs_transtool,sigma=0.07)
    ...
    if head:
        ...
        np.savetxt(simcatafilename, dd, fmt=["%d","%d","%.4f","%.4f","%.4f","%.4f","%.4f","%.4f","%.4f","%.16f","%.16f","%.16f","%.4f","%.4f","%.4f","%.4f","%.4f","%.4f","%.4f","%.4f","%.4f","%d"])
...
def getOneNightCatalog(catano, ccd, destdir, templatefile, rac, decc, sqd, pixscal, sizes, zoneheig):
    """
    ...
    """
    wcs=genwcs(rac,decc,pixscal,sizes)
    genOneSimuCatalog(catano, ccd, destdir, templatefile, rac, decc, sqd, wcs, pixscal, sizes, zoneheig)
...
def star_generator(**workparams):
    ...
    pool.map(functools.partial(getOneNightCatalog, ccd=workparams['ccdno'], destdir=workparams['destdir'], templatefile=workparams['templatefile'], rac=workparams['rac'], decc=workparams['decc'], sqd=workparams['sqd'], pixscal=workparams['pixscal'], sizes=workparams['sizes'], zoneheig=workparams['zoneheight']), range(workparams['n1cata'], workparams['n2cata']+1)) 
